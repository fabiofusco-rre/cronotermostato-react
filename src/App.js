...
const App = () => {
  ...
  useEffect(() => {
    ...
    const urlGetConfig =
      localStorage.getItem("urlGetConfig") || "/apiserver/config";
    fetch(urlGetConfig)
      .then((res) => res.json())
      .then(
        (data) => {
    ...
    const urlGetClimateSensors =
      localStorage.getItem("urlAPIStates") || "/api/states";
    ...
    const urlGetTemperatureSensors =
      localStorage.getItem("urlAPIStates") || "/api/states";
      ...
      .then(
        (data) => {
          ...
          const r = data.filter((el) =>
            el.entity_id.startsWith("temperature.")
          );
          //console.log('r',r)
          ...
          <Grid container spacing={2}>
            ...
            {appConfig.status === false && (
              <Grid item xs={12}>
                <h2 style={{ textAlign: "center" }}>
                  Cronotermostato disattivato, acceddere alle impostazioni ({" "}
                  <span onClick={handleSettingsOpen}>
                    <SettingsIcon />
                  </span>
                  ) per attivarlo.
                </h2>
            ...
            {appConfig.vacationMode && appConfig.status && (
              <Grid item xs={12}>
                ...
                <h2 style={{ textAlign: "center" }}>
                  Cronotermostato in modalità vacanza ({" "}
                  {appConfig.vacationTemperature}°), acceddere alle impostazioni
                  ({" "}
                  <span onClick={handleSettingsOpen}>
                    <SettingsIcon />
                  </span>
